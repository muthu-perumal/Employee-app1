...
const createEmployee = async (req, res) => {
  try {
    ...
    const {
      ...
      CL_allocated, EL_allocated, SL_allocated
    } = req.body;
    if (
      !first_name ||
      !last_name ||
    ...
    await LeaveBalance.create({
      ...
      year,
      CL_allocated: CL_allocated || 0,
      EL_allocated: EL_allocated || 0,
      SL_allocated: SL_allocated || 0,
      CL_available: CL_allocated || 0,
      EL_available: EL_allocated || 0,
      SL_available: SL_allocated || 0,
    });
...
const editEmployee = async (req, res) => {
  try {
    ...
    // Update or Create Leave Balance for the current year
    const year = new Date().getFullYear();
    const balance = await LeaveBalance.findOne({ employeeId: employee_id, year });
    if (balance) {
      // Calculate new available balance by adjusting for used leave
      balance.CL_allocated = CL_allocated || 0;
      balance.EL_allocated = EL_allocated || 0;
      balance.SL_allocated = SL_allocated || 0;
      balance.CL_available = balance.CL_allocated - (balance.CL_used || 0);
      balance.EL_available = balance.EL_allocated - (balance.EL_used || 0);
      balance.SL_available = balance.SL_allocated - (balance.SL_used || 0);
      await balance.save();
    } else {
      await LeaveBalance.create({
        employeeId: employee_id,
